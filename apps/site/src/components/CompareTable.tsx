...
export default function CompareTable({ sources, technologies, initialIds, base }: Props) {
  ...
  return <div class="compare-builder">
    ...
    {chosen.length === 0 ? <p class="empty-state">비교할 정보원을 최대 4개 선택하세요.</p> : <div class="table-scroll"><table><caption>선택한 정보원 비교</caption><thead><tr><th scope="col">항목</th>{chosen.map((source) => <th scope="col"><a href={`${base}/sources/${source.id}/`}>{source.name}</a><button class="text-button" onClick={() => setIds((current) => removeFromComparison(current, source.id))}>제거</button></th>)}</tr></thead><tbody>
